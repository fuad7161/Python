...
from okkhor import number_to_word as numeric_words
from okkhor import number_to_word_units as units
...
def generate_segments(number):
    ...
    """
    segments = dict()
    segments['quadtrilion'] = math.floor(number/ 10000000000000)
    number = number % 10000000000000
    segments['trilion'] = math.floor(number/ 100000000000)
    number = number % 100000000000
    segments['bilion'] = math.floor(number/ 1000000000)
...
def whole_part_word_gen(segments):
    ...
    for segment in segments:
        if segments[segment]:
            generated_words += numeric_words[str(segments[segment])] + " " + units[segment] + " "
